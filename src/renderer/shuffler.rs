//! RGB shuffler renderer. Made because Cairo infuriatingly uses ARGB instead of RGBA, messing up
//! all GPUs in the process.
//!
//! Stolen from https://github.com/parasyte/pixels/blob/main/examples/custom-shader/src/renderers.rs

use pixels::wgpu::{self, util::DeviceExt};

pub(crate) struct ShufflingRenderer {
...
impl ShufflingRenderer {
    pub(crate) fn new(pixels: &pixels::Pixels, width: u32, height: u32) -> Self {
        let device = pixels.device();
        let shader = wgpu::include_wgsl!("shaders/shuffle.wgsl");
        ...
        let sampler = device.create_sampler(&wgpu::SamplerDescriptor {
            label: Some("ShufflingRenderer sampler"),
        ...
        let vertex_buffer = device.create_buffer_init(&wgpu::util::BufferInitDescriptor {
            label: Some("ShufflingRenderer vertex buffer"),
        ...
        let pipeline_layout = device.create_pipeline_layout(&wgpu::PipelineLayoutDescriptor {
            label: Some("ShufflingRenderer pipeline layout"),
            bind_group_layouts: &[&bind_group_layout],
            push_constant_ranges: &[],
        });
        let render_pipeline = device.create_render_pipeline(&wgpu::RenderPipelineDescriptor {
            label: Some("ShufflingRenderer pipeline"),
    ...
    pub(crate) fn render(
        ...
        let mut rpass = encoder.begin_render_pass(&wgpu::RenderPassDescriptor {
            label: Some("ShufflingRenderer render pass"),
